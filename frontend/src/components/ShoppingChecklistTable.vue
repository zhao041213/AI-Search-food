<template>
  <section class="shopping-checklist" aria-label="采购清单">
    ...
    <el-table class="shopping-table" :data="items" size="large">
      ...
      <el-table-column v-if="showInventory" label="入库" width="112" fixed="right">
        <template #default="scope">
    ...
    <div class="shopping-mobile-list">
      <article v-for="item in items" :key="item.name" class="shopping-mobile-card">
        <div class="mobile-card-heading"><strong>{{ item.name }}</strong><span>{{ item.amount || '用量待确认' }}</span></div>
        <div class="mobile-card-meta"><span :class="['inventory-state', `inventory-${inventoryStatus(item).toLowerCase()}`]">{{ inventoryLabel(item) }}</span><span>{{ stateLabel(item) }}</span></div>
...
<style scoped>
.shopping-checklist { min-width: 0; }
...
.status-button { display: inline-flex; align-items: center; gap: 4px; min-height: 36px; padding: 0; border: 0; color: var(--app-text-soft); background: transparent; cursor: pointer; }
...
.purchase-links { display: flex; flex-wrap: wrap; gap: 7px; }
.purchase-links a { display: inline-flex; align-items: center; gap: 4px; min-height: 28px; padding: 0 7px; border: 1px solid var(--app-line-strong); border-radius: 4px; color: var(--app-text-soft); background: var(--app-surface); font-size: 11px; font-weight: 800; text-decoration: none; }
.inventory-cell { display: grid; gap: 3px; }
...
.shopping-mobile-list { display: none; }
@media (max-width: 767px) { .shopping-table { display: none; } .shopping-mobile-list { display: grid; gap: 8px; } .shopping-mobile-card { display: grid; gap: 8px; padding: 12px; border: 1px solid var(--app-line); border-radius: 8px; background: var(--app-surface); } .mobile-card-heading,.mobile-card-meta { display: flex; justify-content: space-between; gap: 8px; } .mobile-card-heading strong { color: var(--app-text); } .mobile-card-heading span,.mobile-card-meta { color: var(--app-text-muted); font-size: 12px; } }
</style>
